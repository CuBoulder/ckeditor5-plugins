...
export default class BoxCommand extends Command {
	refresh() {
		...
		if (firstRange.isCollapsed) {
			if (selection.hasAttribute('box')) {
				const attributeValue = selection.getAttribute('ucb-box');

				// Find the entire range containing the tooltip under the caret position.
				const tooltipRange = findAttributeRange(selection.getFirstPosition(), 'ucb-box', attributeValue, model);
		...
		// or an entire tooltip.
			...
			if (selection.hasAttribute('ucb-box')) {
				const attributeValue = selection.getAttribute('ucb-box');
		...
		this.isEnabled = model.schema.checkAttributeInSelection(selection, 'ucb-box');
	}
	...
	execute({ abbr, title }) {
		...
		model.change(writer => {
			...
			if (selection.isCollapsed) {
				...
				writer.removeSelectionAttribute('ucb-box');
			} else {
				...
				const ranges = model.schema.getValidRanges(selection.getRanges(), 'ucb-box');

				for (const range of ranges) {
					writer.setAttribute('ucb-box', title, range);
				}
			}
		});
	}
